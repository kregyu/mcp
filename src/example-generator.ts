...
import type { ComponentAPI, UsageExample, PropDefinition } from './types.js';
...
export class KPCExampleGenerator {
  ...
  generateBasicExample(component: ComponentAPI): UsageExample {
    ...
    if (propsCode) templateCode += `\n  ${propsCode}`;
    if (eventsCode) templateCode += `\n  ${eventsCode}`;
  ...
  generateFormExample(component: ComponentAPI): UsageExample | null {
    if (!this.isFormComponent(component)) return null;
...
export default {
  ...
  generateAdvancedExample(component: ComponentAPI): UsageExample {
    ...
    const eventsCode = component.events.map(event => 
      `@${event.name}="handle${this.capitalize(event.name)}"`
    ).join('\n  ');
    ...
    const methodsCode = component.events.map(event => 
      `    handle${this.capitalize(event.name)}(${this.getEventParams(event)}) {
...
${methodsCode}
   ...
   * 生成事件处理示例
   */
  private generateEventExample(component: ComponentAPI): UsageExample {
...
${methodsCode}
   ...
   * 选择基础属性
   */
  private selectBasicProps(props: PropDefinition[]): PropDefinition[] {
    // 优先选择必需属性和常用属性
   ...
   * 生成属性代码
   */
  private generatePropsCode(props: PropDefinition[]): string {
   ...
   * 生成事件代码
   */
  private generateEventsCode(events: any[]): string {
   ...
   * 生成脚本代码
   */
  private generateScriptCode(component: ComponentAPI, props: PropDefinition[], events: any[]): string {
    const methodsCode = events.length > 0 ? `,
  
  methods: {
    ${events.map(event => 
      `handle${this.capitalize(event.name)}() {
...
export default {
   ...
   * 获取属性值
   */
  private getPropValue(prop: PropDefinition): string {
   ...
   * 获取高级属性值
   */
  private getAdvancedPropValue(prop: PropDefinition): string {
   ...
   * 获取默认数据值
   */
  private getDefaultDataValue(component: ComponentAPI): string {
   ...
   * 判断是否为表单组件
   */
  private isFormComponent(component: ComponentAPI): boolean {
   ...
   * 生成表单组件代码
   */
  private generateFormComponentCode(component: ComponentAPI): string {
   ...
   * 获取表单默认值
   */
  private getDefaultFormValue(component: ComponentAPI): string {
    const valueProp = component.props.find(p => p.name === 'value');
    if (!valueProp) return "''";
   ...
   * 生成验证规则
   */
  private generateValidationRules(component: ComponentAPI): string {
   ...
   * 生成高级数据属性
   */
  private generateAdvancedDataProperties(props: PropDefinition[]): string {
    return props.map(prop => {
      const value = this.getAdvancedPropValue(prop);
   ...
   * 获取事件参数
   */
  private getEventParams(event: any): string {
    if (!event.parameters || event.parameters.length === 0) {
      return '';
    }
    return event.parameters.map((p: any) => p.name || 'event').join(', ');
  }

  /**
   * 生成插槽内容
   */
  private generateSlotContent(component: ComponentAPI): string {
    switch (component.name) {
      case 'Button':
        return '点击按钮';
      case 'Card':
   ...
   * 检查是否有需要响应式数据的属性
   */
  private hasReactiveProps(props: PropDefinition[]): boolean {
