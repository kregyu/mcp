...
class KPCMCPTester {
  ...
  private async testDataLoader(): Promise<void> {
    console.log('📊 测试数据加载器...');
  ...
  private async testComponentRetrieval(): Promise<void> {
  ...
  private async testExampleGeneration(): Promise<void> {
  ...
  private async testFormatting(): Promise<void> {
    ...
    await this.runTest('格式化验证结果', async () => {
      ...
      const invalidResult = { 
        isValid: false, 
        errors: [{ type: 'unknown-prop' as const, message: '测试错误' }], 
        suggestions: ['测试建议'] 
      };
