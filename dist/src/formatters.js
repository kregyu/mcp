...
\`\`\``;
     ...
     * 格式化组件列表
     */
    formatComponentList(components, title = '组件列表') {
        ...
        const sections = Array.from(grouped.entries()).map(([category, comps]) => {
            const componentItems = comps.map(comp => `- **${comp.name}**: ${comp.description}`).join('\n');
     ...
     * 格式化搜索结果
     */
    formatSearchResults(components, query) {
...
${categoryItems}`;
     ...
     * 格式化快速参考
     */
    formatQuickReference(component) {
