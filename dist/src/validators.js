...
export class KPCUsageValidator {
     ...
     * 验证组件使用是否正确
     */
    validate(component, props, context) {
        const errors = [];
        const suggestions = [];
        // 1. 验证必需属性
     ...
     * 验证必需属性
     */
    validateRequiredProps(component, props, errors, suggestions) {
     ...
     * 验证属性是否存在
     */
    validatePropExists(component, props, errors, suggestions) {
     ...
     * 验证属性类型（简化版）
     */
    validatePropTypes(component, props, errors, suggestions) {
     ...
     * 验证嵌套关系
     */
    validateNesting(component, context, errors, suggestions) {
     ...
     * 验证特殊规则
     */
    validateSpecialRules(component, props, context, errors, suggestions) {
        // FormItem特殊规则
     ...
     * 查找相似的属性名
     */
    findSimilarProp(target, propNames) {
        const threshold = 0.6;
        let bestMatch = null;
        let bestScore = 0;
